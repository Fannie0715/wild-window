...
小红书小工具容器禁止联网和 iframe，无法直接运行远程直播。离线小游戏使用独立的 HTML/CSS/JS 资源包；它与直播版分开，不能把离线内容标为实时直播或 AI 自动识别。规则与打包说明见 `docs/xiaohongshu.md`，玩法说明见 `docs/offline-game.md`。小游戏源码在 `public/observation-game/`；运行本地网页后，从页脚打开“离线观察挑战”。
