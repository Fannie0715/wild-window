...
export default function Home() {
  ...
  useEffect(() => { miniWindow.current = new URLSearchParams(location.search).has('mini'); setCompact(miniWindow.current); setClock(new Date()); const timer = setInterval(() => setClock(new Date()), 1000); return () => clearInterval(timer); }, []);
  ...
  async function changeCompact(next: boolean) {
    if (resizing) return;
    setResizing(true);
    try {
      const native = desktopBridge();
      if (native) await native.setCompact(next);
      else if (window.opener) window.resizeTo(next ? 360 : 1180, next ? 320 : 860);
      setCompact(next);
      const url = new URL(location.href);
      if (next) url.searchParams.set('mini', '1'); else url.searchParams.delete('mini');
      history.replaceState(history.state, '', url);
      setNotice('');
    } catch { setNotice('窗口调整失败，请关闭窗口后重新打开。'); }
    finally { setResizing(false); }
  }
  function openMini() {
    if (miniWindow.current) { void changeCompact(true); return; }
    const native = desktopBridge();
    if (native) { void native.openMini(camera.id).catch(() => setNotice('小窗打开失败，请重试。')); return; }
    const target = new URL(location.href);
    target.searchParams.set('mini', '1'); target.searchParams.set('camera', camera.id);
    const win = window.open(target.toString(), 'wild-window-mini', 'popup,width=360,height=320');
    if (!win) { miniWindow.current = true; void changeCompact(true).then(() => setNotice('已切换为紧凑模式；允许弹出窗口后可独立显示。')); }
  }
    ...
    <div className="workspace">
      <section className="main-column">
        ...
        <div className="monitor" ref={monitor}>
          <div className="monitor-top"><div><Radio size={15}/><span>GLOBAL WILDLIFE</span><span className="muted"> / MONITOR {String(index+1).padStart(2,'0')}</span></div><div className="monitor-top-actions"><div className="monitor-led"><span className="status-dot"/>{playing ? compact ? '播放器' : 'OFFICIAL PLAYER' : compact ? '待机' : 'STANDBY'}</div>{compact && <button className="icon-button compact-expand" aria-label="展开控制台" title="展开控制台" disabled={resizing} onClick={() => void changeCompact(false)}><Maximize size={13}/></button>}</div></div>
