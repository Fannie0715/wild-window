...
test('local distribution serves its build and does not expose files outside it', async t => {
  ...
  await writeFile(join(root,'index.html'),'<h1>Global Wildlife Monitor</h1>');
  ...
  const html=await fetch(base+'/?mini=1');assert.equal(html.status,200);assert.match(await html.text(),/Global Wildlife Monitor/);
